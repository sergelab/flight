...
- Управление: ↑/↓ — газ (throttle), ←/→ — поворот (turn), Q/A — вверх/вниз по высоте, Esc — выход.
...
- `height_offset` — текущий отступ от поверхности (units)
...
## Вертикальный контроль (Q/A)

Ввод `lift` (Q/A) плавно изменяет `height_offset`:

- `height_offset += lift * climb_rate * dt`

Нижняя граница — поверхность и вода:
- нельзя опуститься ниже `max(height_at(x,z), water_level)`.
...
- `--climb-rate` — скорость набора/снижения высоты (units/sec)
