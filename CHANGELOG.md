...
## v0.8.0 (Manual controls + --auto)
- Добавлено ручное управление полётом стрелками: ↑/↓ (вперёд/назад), ←/→ (поворот).
- Добавлен CLI флаг `--auto`, который включает прежний режим автополёта вперёд.
- CLI: `--turn-rate` (скорость поворота в рад/сек) для ручного режима.
- Обновлена документация: README и `docs/tech/build_run.md`.

### Как проверить
1) `poetry install`
2) Ручной режим: `poetry run flight`
   - проверьте, что ↑/↓ двигают вперёд/назад, а ←/→ поворачивают камеру
3) Авто режим: `poetry run flight --auto`
   - проверьте, что камера постоянно летит вперёд без нажатия клавиш

## v0.7 (Forests: 3D trees, Variant C)
