...
## v0.9.3 (Fix: bank direction)

- Исправлено направление наклона (bank): при поворотах влево/вправо камера теперь наклоняется в корректную сторону.

### Как проверить
1) `poetry install`
2) `poetry run flight --debug`
3) Нажмите и удерживайте ←, затем →: крен должен быть "внутрь" соответствующего поворота.

## v0.9.2 (Heavier controls: less "snappy" by default)

- Управление стало заметно менее резким по умолчанию:
  - снижены дефолты ускорения/торможения и угловых ускорений, чтобы инерция ощущалась без тюнинга.
- Добавлены параметры и реализация для восприятия "веса" с цифровым вводом:
  - сглаживание стрелок (`--input-smooth`)
  - лаг yaw камеры (`--cam-yaw-smooth`)
  - лёгкий визуальный pitch при разгоне/торможении (`--pitch-*`)
- Документация обновлена: tech-доки и примеры CLI.

### Как проверить
1) `poetry install`
2) `poetry run flight --debug`
3) Проверьте, что:
   - скорость разгоняется/гасится более плавно (HUD `v=`)
   - повороты набирают/гасят `yaw_rate` плавно, после отпускания есть "докатка"
   - камера заметно менее "дёргается" от стрелок
4) Для сравнения: `poetry run flight --debug --input-smooth 10 --cam-yaw-smooth 10 --accel 10 --yaw-accel 6` (более резкий контроль)

## v0.9.0 (Core Flight Feel: inertial manual flight)

- Fix: добавлен `flight/util/__init__.py`, чтобы модуль `flight.util` корректно импортировался при установке через Poetry.
